...
## Purpose
List, preview, download, and delete files stored in Backblaze B2 — the
**full-bucket** view (source media, derived artifacts, everything). For a view
restricted to just the derived runs under `DATASET_PREFIX`, see the scoped
[Mask Dataset](mask-dataset.md) explorer at `/dataset`. Both views coexist:
`/files` is the unscoped bucket explorer; `/dataset` is the sample-specific one.
