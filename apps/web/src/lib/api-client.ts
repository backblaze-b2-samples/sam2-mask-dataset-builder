...
export async function segmentImage(source_key: string, objects: ObjectPrompt[]) {
  return apiFetch<SegmentationRun>("/segment/image", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ source_key, objects }),
  });
}

/** Run video propagation (heavier, GPU-recommended). */
export async function segmentVideo(
  ...
  return apiFetch<SegmentationRun>("/segment/video", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ source_key, objects, prompt_frame, max_frames }),
  });
}

export async function getRuns(limit = 100) {
  return apiFetch<SegmentationRun[]>(`/runs?limit=${limit}`);
}

export async function getRun(runId: string) {
  return apiFetch<SegmentationRun>(`/runs/${runId}`);
}

export async function getDatasetSummary() {
  return apiFetch<DatasetSummary>("/dataset/summary");
}

export async function getDatasetFiles(prefix = "", limit = 1000) {
  return apiFetch<FileMetadata[]>(
    `/dataset/files?prefix=${encodeURIComponent(prefix)}&limit=${limit}`,
  );
}
