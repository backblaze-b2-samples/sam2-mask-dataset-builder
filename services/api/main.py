...
async def lifespan(_app: "FastAPI"):
    missing = [env_name for attr, env_name in REQUIRED_B2_SETTINGS if not getattr(settings, attr)]
    if missing:
        raise RuntimeError(
...
app = FastAPI(
    title="SAM 2 Mask Dataset Builder API",
    description=(
        "Promptable SAM 2 segmentation over a Backblaze B2 media archive — "
        "ingest source media, run masks, and write a labeled dataset back to B2"
    ),
    version="0.1.0",
    lifespan=lifespan,
)
