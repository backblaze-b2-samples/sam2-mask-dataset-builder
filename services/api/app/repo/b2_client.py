...
def get_s3_client():
    return boto3.client(
        ...
        config=Config(
            ...
            user_agent_extra="b2ai-sam2-mask-builder",
        ),
    )
...
def upload_file(file_data: bytes, key: str, content_type: str) -> FileMetadata:
...
def download_file(key: str) -> bytes:
    ...
    Used by the segmentation engine, which needs the source image/video bytes
    to run inference. All B2 access stays inside this repo module.
    """
    client = get_s3_client()
    try:
...
def get_upload_stats(prefix: str = "") -> dict:
    """Paginate through all objects (optionally under a prefix) and return
    aggregate stats. Raises RuntimeError on S3 failure.
    """
    client = get_s3_client()
    contents: list[dict] = []
